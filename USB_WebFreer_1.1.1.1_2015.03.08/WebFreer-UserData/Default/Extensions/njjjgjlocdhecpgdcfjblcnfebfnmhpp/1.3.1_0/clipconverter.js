...
function addButtons() {
	...
	var clipconverterpath = 'http://www.clipconverter.cc/?ref=addon&version=131&browser=chrome&url=' + encodeURIComponent(document.URL);
	...
	if(document.getElementById('watch-headline-user-info')) {
		...
		var spanclass="yt-uix-button-context-light";
	}
	...
	var result = /"url_encoded_fmt_stream_map": "(.*?)"/.exec(document.body.innerHTML);
	if(result) {
		urlmap = (result[1]);
		var result2 = /"adaptive_fmts": "(.*?)"/.exec(document.body.innerHTML);
		if(result2)
			urlmap +=  "," + (result2[1]);
	...
	if(div_embed) {
		...
		setInterval(function(){
			if(!document.getElementById('clipconverter')) {
				addButtons();
			}
		}, 1000);
